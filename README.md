...
Use image revision tags such as `wodby/mariadb:11.4-rN` to select a Wodby image revision.
Major and minor tags use the repository release number. Full-version tags such as
`wodby/mariadb:11.4.13-r0` start at `r0` for each exact upstream version.
...
See [release tags](https://github.com/wodby/mariadb/tags) for available revisions and the [image revision policy](https://github.com/wodby/images#image-revisions) for upgrade guidance.
...
Credits to the Alpine Linux team for patches for better musl compatibility of MariaDB. Patches taken from
Alpine's [package repository](https://pkgs.alpinelinux.org/packages).
